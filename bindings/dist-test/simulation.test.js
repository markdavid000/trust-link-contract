...
import { ContractInvokeError } from "./errors.js";
import { assertSimulationSucceeds, createEscrowSimulator, isSimulationError, simulateAndCatch, } from "./simulation.js";
...
test("simulateAndCatch returns the expected error for a thrown ContractInvokeError", async () => {
    const result = await simulateAndCatch(() => {
        throw new ContractInvokeError(3 /* ErrorCode.EscrowNotFound */);
    });
    assert.equal(result.ok, false);
    if (!result.ok) {
        assert.ok(result.error instanceof ContractInvokeError);
        assert.equal(result.code, 3 /* ErrorCode.EscrowNotFound */);
...
test("simulateAndCatch parses a raw Soroban contract error string", async () => {
    ...
    assert.equal(result.ok, false);
    if (!result.ok) {
        assert.equal(result.code, 5 /* ErrorCode.NotAuthorized */);
        assert.ok(result.error instanceof ContractInvokeError);
    }
});
test("simulateAndCatch parses a { code } shaped contract error", async () => {
    const result = await simulateAndCatch(() => Promise.reject({ code: 14 /* ErrorCode.ContractPaused */ }));
    assert.equal(result.ok, false);
    if (!result.ok)
        assert.equal(result.code, 14 /* ErrorCode.ContractPaused */);
});
...
test("isSimulationError narrows the failure variant", async () => {
    const result = await simulateAndCatch(() => {
        throw new ContractInvokeError(1 /* ErrorCode.InvalidAmount */);
    });
...
test("assertSimulationSucceeds returns the value or throws the expected error", async () => {
    ...
    await assert.rejects(() => assertSimulationSucceeds(() => {
        throw new ContractInvokeError(24 /* ErrorCode.DisputeWindowClosed */);
    }), (err) => err instanceof ContractInvokeError && err.code === 24 /* ErrorCode.DisputeWindowClosed */);
});
test("createEscrowSimulator simulates calls through a transport", async () => {
    const transport = {
        invoke(method) {
            if (method === "get_arbitration_fee")
                return 250;
            throw new ContractInvokeError(5 /* ErrorCode.NotAuthorized */);
        },
    };
    const sim = createEscrowSimulator(transport);
    const ok = await sim.simulate("get_arbitration_fee", []);
    assert.equal(ok.ok, true);
    if (ok.ok)
        assert.equal(ok.value, 250);
    const failed = await sim.simulate("withdraw_fees", []);
    assert.equal(failed.ok, false);
    if (!failed.ok)
        assert.equal(failed.code, 5 /* ErrorCode.NotAuthorized */);
});
