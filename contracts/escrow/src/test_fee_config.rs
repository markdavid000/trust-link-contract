...
fn test_fee_updates_are_independent() {
    ...
    client.set_arbitration_fee(&250_u32);
    client.set_protocol_fee(&1000_u32);

    let config = client.get_fee_config();
    assert_eq!(config.protocol_fee_bps, 1000);
    assert_eq!(config.arbitration_fee_bps, 250);

    client.set_arbitration_fee(&500_u32);
...
fn test_fee_bounds_are_enforced() {
    ...
    let protocol_result = client.try_set_protocol_fee(&10_001_u32);
    assert!(matches!(protocol_result, Err(Ok(ContractError::FeeExceedsMax))));

    let arbitration_result = client.try_set_arbitration_fee(&10_001_u32);
...
fn test_fee_config_persists_across_calls() {
    ...
    client.set_protocol_fee(&600_u32);
    client.set_arbitration_fee(&300_u32);
